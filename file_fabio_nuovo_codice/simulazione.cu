...
void simulazione(
    ...
    if(model_weights_h==nullptr) model_weights_h = (float*) malloc(tot_models_weight_size);
    else model_weights_h       = weights_models;
    if(model_biases_h==nullptr)  model_biases_h = (float*) malloc(tot_models_bias_size);
    ...
    if(biases_models==nullptr && weights_models==nullptr){
        ...
        launch_fill_random_kernel(model_weights_d,n_weight*n_creature,-2.0f,2.0f,0,streams[0]);
        CUDA_CHECK(cudaGetLastError());
        launch_fill_random_kernel(model_biases_d,n_bias*n_creature,-2.0f,2.0f,0,streams[0]);
        CUDA_CHECK(cudaGetLastError());
    ...
    for (int epoca = 0; epoca < N_EPHOCS; epoca++) {
        ...
        for(int step=0; step<N_STEPS; step++){
            ...
            // - Aggiornamento matrice signaling 
